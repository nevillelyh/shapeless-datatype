...
class BigQueryType[A] extends Serializable {
  def fromTableRow[L <: HList](
    m: TableRow
  )(implicit gen: LabelledGeneric.Aux[A, L], fromL: FromTableRow[L]): Option[A] =
    fromL(m.asInstanceOf[BigQueryMap]).map(gen.from)
...
object BigQueryType {
  ...
  def at[V: TypeTag](
    typeName: String
  )(fromFn: Any => V, toFn: V => Any): BaseBigQueryMappableType[V] = {
